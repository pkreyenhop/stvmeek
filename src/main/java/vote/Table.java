...
public class Table {
    ...
    private static final int RANK_ORDERING = 1;
    private static final int CAND_ORDERING = 2;
    private final Status mSys;
    public int[][] ranks;
    //int[] candkey;
    public int numvoters, numcandidates;
    private String[] mCandidates;
    private int theType;
    private Vector voteIDs;
    ...
    private void validateRankOrder() {
        int invalid = 0;
        ...
        for (int from = 0; from < numvoters; from++) {
            if (valid[from]) {
                System.arraycopy(ranks[from], 0, ranks2[to], 0, numcandidates);
                to++;
            } else {
                voteIDs.remove(to);
            }
    ...
    private void candOrderToRankOrder() {
        int temp[] = new int[numcandidates];
        for (int i = 0; i < numvoters; i++) {
            for (int j = 0; j < numcandidates; j++) {
                temp[j] = -1;
            ...
            System.arraycopy(temp, 0, ranks[i], 0, numcandidates);
            //System.out.println("");
        }
    }
